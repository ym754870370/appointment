...
router.get('/', async (ctx) => {//ctx：上下文context,包含了response和request等信息
  ctx.body = "首" // 返回数据， 相当于 原生中的 res.writeHead()  res.end()

  //let userInfo = new Buffer(data, 'base64').toString(); //汉字不可以设置cookie  value，可以用转码的方法解决

  ctx.cookies.set('userInfo', 'yangming', {
    maxAge: 60*1000*60
  })
}).get('/news', async(ctx) => {
  ctx.body = "新闻页"
